...
bigram_counts <- subset(petisi_words, select = c(2, 3)) %>%
  separate(kata, into = c("word1", "word2"))
...
Berikut ini adalah term dengan probabilitas tertinggi sebagai topik yang dibahas dalam petisi tentang gojek.   

```{r, fig.cap="Hasil topic modelling"}
tm_petisi <- subset(petisi_words, select = c(2, 3))

tm_petisi_gojek <- bigram_tm(tm_petisi$kata, number_of_topics = 4)
tm_petisi_gojek
```

Berdasarkan topic modelling di atas, term yang muncul dalam topic 1 dapat diinterpretasikan sebagai pandangan masyarakat tentang transprtasi onlin. Dalam konteks ini, kemungkinan mereka menganggap bahwa jenis transportasi online dapat membantu dan dibutuhkan oleh masyarakat. Oleh karena itu pula, dalam topic 2 dapat diinterpretasikan sebagai alasan mereka menandatangani petisi. Alasannya kemungkinan besar karena mereka menganggap pelarangan transportasi online justru "merugikan" masyarakat. Alasan lebih lanjut, dapat ditemui di hasil topic modelling 3 dan 4. Di mana di sana mereka juga menyatakan mendukung gojek karena dianggap dapat membuka "lapangan pekerjaan" serta "nyaman." Satu hal lagi, di sini, masyaraat juga menggap bahwa "online" memudahkan mereka untuk mendapatkan transportasi.   

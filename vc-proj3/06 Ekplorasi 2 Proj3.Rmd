...
#net_telukBenua <- read_csv(paste(dirwd,"net_tb_clean.csv",sep=''), col_names = TRUE)
...
User terlibat di ambil berdasarkan frekuensi keberadaan user baik sebagai akun pengirim twit maupun akun yang dimention di dalam twit. Gambar diatas merupakan hasil perhitungan dari twit yang sebelumnya telah di filter, yaitu twit yang melibatkan minimal dua username. 
...
Tujuan utama dibuat `network of term` adalah untuk mengetahui pasangan kata yang lebih jauh dari bigram. Dalm konteks ini, pasangan kata dengan garis paling tebal kurang lebih sama dengan hasil yang ditunjukkan dalam visualisasi frekeunsi bigram di atas. Namun di sini, kita juga dapat melihat, misalnya kata **TV** itu berpasangan bukan hanya dengan **Metro** tapi juga dengan **kompas** yang merujuk pada kompasTV. Artinya, kemungkinan besar cerita tentang bali tolak reklamasi ini juga dimiliki oleh kompas tv atau aktor BTR juga diundang oleh tv tersebut. 

**Hint: interpretasi dilakukan dengan visualisasi saja, yaitu dengan melihat kata-kata yang ada dalam satu garis.** 
...
forbali13 <- tb_raw %>%
  filter(is_duplicate == FALSE) %>%
  filter(str_detect(user, "ForBALI13")) %>%
  select(date,tweets, periode) %>%
  print(tweets)

twit_aktor <- bind_rows(gendovara %>%
                          mutate(person = "gendovara"), 
                        JRX_SID %>%
                          mutate(person = "JRX_SID"), 
                        forbali13 %>%
                          mutate(person = "forbali13"))

write_csv(twit_aktor, path = "wrangled data proj-3/twit_aktor.csv")
```

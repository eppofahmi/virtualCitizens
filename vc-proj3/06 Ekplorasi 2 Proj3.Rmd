...
tb_raw %>%
  filter(is_duplicate == FALSE) %>%
  filter(!str_detect(tweets, "RT")) %>%
  filter(str_detect(clean_text, "part")) %>%
  arrange(desc(ret_count)) %>%
  select(user, date, tweets) %>%
  head(n = 5) %>% 
  kable()
...
### mencari term dalam periode 1 (twit yang berasal dari tahun 2013)

```{r}
tb_raw %>%
  filter(is_duplicate == FALSE) %>%
  filter(periode == "periode_1") %>%
  filter(str_detect(clean_text, "hebat bener")) %>%
  select(user, date, tweets) %>%
  head(n = 5) %>% 
  kable()
```
...
tb_raw %>%
  ...
  filter(periode == "periode_3") %>%
  filter(!str_detect(tweets, "RT")) %>%
  filter(str_detect(tweets, "Parade Budaya")) %>%
  select(user, date, tweets) %>%
  head(n = 5) %>% 
  kable()
```
...
tb_raw %>%
  ...
  filter(periode == "periode_3") %>%
  filter(!str_detect(tweets, "RT")) %>%
  filter(str_detect(clean_text, "pantai")) %>%
  select(user, date, tweets) %>%
  head(n = 5) %>% 
  kable()
```

Berdasarkan penuturan partisipan penelitian, revitalisasi merupakan salah satu propaganda pendukung reklamasi. Pada periode tiga ini kita juga dapat melihat bahwa revitalisasi dianggap sebagai kedok. Beberapa twit tentang hal tersebut adalah sebagai berikut. 
...
tb_raw %>%
  ...
  filter(periode == "periode_3") %>%
  filter(!str_detect(tweets, "RT")) %>%
  filter(str_detect(clean_text, "konser")) %>%
  select(date, tweets) %>%
  print() %>%
  head(n = 10) %>% 
  kable()
